...
//  FBSnapshotTestCase
//
//  Created by wang su on 2024/6/5.
//
...
public extension UIColor {
    ...
    /// 把颜色转成图片
    func sw_toImage(size: CGSize) -> UIImage{
    ...
    var sw_hex: Int {
        let red = Int(self.sw_rgba.0 * 255) << 16
        let green = Int(self.sw_rgba.1 * 255) << 8
        let blue = Int(self.sw_rgba.2 * 255)
        return red+green+blue
    }
    var sw_rgba: (CGFloat, CGFloat, CGFloat, CGFloat) {
    ...
    /// 16进制转UIColor
    static func sw_color(hex: String, alpha: CGFloat = 1.0) -> UIColor {
